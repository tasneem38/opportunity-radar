...
from crewai import LLM  # imported for native Gemini & Sarvam support
    ...
    async def run_filing_pipeline(self):
        ...
        try:
            ...
            try:
                ...
                if start_idx != -1:
                    clean_json_str = str(clean_json)
                    clean_json = clean_json_str[start_idx:]  # type: ignore
                ...
                for s in signals:
                    ...
                    symbol = str(symbol or "")[:20].strip().upper()  # type: ignore
                    
                    logger.debug(f"Processing symbol: {symbol}")
                    res = db.client.table("stocks").select("id").eq("symbol", symbol).execute()
                    data_list = cast(list, res.data)
                    stock_id = data_list[0]["id"] if data_list else None
                    
                    if not stock_id:
                        logger.info(f"Resolving proper name for new symbol: {symbol}")
                        company_name = YFinanceClient.resolve_company_name(symbol)
                        
                        company_name = str(company_name or "")[:200]  # type: ignore
